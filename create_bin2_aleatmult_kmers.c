...
uint64_t aleat_multiplex(char ch1){
	uint64_t res;
	int i1;
...
*/
	...
	uint64_t mask_first_letter = ((uint64_t)7) << (3*(kmer_size-1));
	uint64_t mask_last_letter = 7;
	uint64_t mask_end =  ( ((uint64_t) 1) << (3*(kmer_size-1)))-1;
	//uint64_t mask_multplex_kmer = (((uint64_t) 1) << (3*(kmer_size)))-1;
...
//				printf("ch1 : %c\n", ch1);
				...
				if (ch == 'N' || ch == 'n'){
					...
					bit_multiplex_char = aleat_multiplex(ch);
					bit_multiplex_kmer <<= 3;
					bit_multiplex_kmer = bit_multiplex_kmer | bit_multiplex_char;
					++actual_kmer_size;
...
//						printf("\nBIT_MULTIPLEX_KMER JUST filled\n\n");
						kmer_start = bit_multiplex_kmer >> 3;
						kmer_end = bit_multiplex_kmer & mask_end;
						first_letter = (bit_multiplex_kmer & mask_first_letter)  >> (3*(kmer_size-1));
...
//						printf("last_letter : %"PRIu64"; mask_last_letter : %"PRIu64"\n", last_letter, mask_last_letter);
	...
	//				printf("ch1 : %c\n", ch1);
				...
				if(ch != 'N' && ch != 'n'){
					...
					bit_multiplex_char = aleat_multiplex(ch);
					bit_multiplex_kmer = ((uint64_t) (bit_multiplex_kmer & mask_end))<<3;
					bit_multiplex_kmer = bit_multiplex_kmer | bit_multiplex_char;
