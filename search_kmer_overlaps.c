...
int main(int argc, char const *argv[])
{
	...
	char kmer_left_part_char[2*length-1];
	char right_letter[3];
	strncpy(kmer_left_part_char, kmer, (2*length)-2);
	kmer_left_part_char[2*length-2] = '\0';
	strncpy(right_letter, &kmer[(2*length)-2], 2);
	right_letter[2] = '\0';

	char kmer_right_part_char[2*length-1];
	char left_letter[3];
	strncpy(kmer_right_part_char, &kmer[2], (2*length)-2);
	kmer_right_part_char[2*length-2] = '\0';
	strncpy(left_letter, kmer, 2);
	left_letter[2] = '\0';
	...
	uint64_t kmer_right_letter = translate_kmer_to_uint64(right_letter, 1);
	uint64_t kmer_left_letter = translate_kmer_to_uint64(left_letter, 1);

	printf("kmer_left_part_uint64 : %"PRIu64"\n", kmer_left_part_uint64);
	printf("kmer_right_part_uint64 : %"PRIu64"\n", kmer_right_part_uint64);

	printf("\nkmer left :\n");
	int nb_left_overlaps = search_kmer_overlaps(ifp, kmer_left_part_uint64, kmer_right_letter, 0);
	printf("\nkmer right :\n");
	int nb_right_overlaps = search_kmer_overlaps(ifp, kmer_right_part_uint64, kmer_left_letter, 1);
